...
function lazyMageLoad.LoadParseMage()
	...
	lazyMage.actions.askl               = lazyMage.Action:New("askl",             "Spell_Arcane_Blast")
	lazyMage.actions.asyd               = lazyMage.Action:New("asyd",             "INV_Enchant_EssenceMysticalLarge")
	...
	function lazyMage.masks.isSwinged()
		if not st_timer then
			lazyMage.p("need SP_SwingTimer.")
			return false
		end
	...
	function lazyScript.bitParsers.ifSwinged(bit, actions, masks)
		...
		table.insert(masks, lazyScript.negWrapper(lazyMage.masks.isSwinged, negate))
		return true
	end
