...
function lazyHunterLoad.LoadParseHunter()
	...
	lazyHunter.actions.aspectWolf		   = lazyHunter.Action:New("aspectWolf",		   "Ability_Mount_WhiteDireWolf", nil, nil, true)
	lazyHunter.actions.baitedShot          = lazyHunter.Action:New("baitedShot",           "inv_misc_food_66")
	...
	lazyHunter.actions.call                = lazyHunter.Action:New("call",                 "Ability_Hunter_BeastCall", nil, nil, true)
	lazyHunter.actions.carve               = lazyHunter.Action:New("carve",                "INV_ThrowingKnife_06")
	...
	lazyHunter.actions.lacerate            = lazyHunter.Action:New("lacerate",             "spell_lacerate_1C")
	...
	lazyHunter.actions.multi               = lazyHunter.Action:New("multi",                "Ability_UpgradeMoonGlaive", nil, nil, true)
	...
	lazyHunter.actions.steady              = lazyHunter.Action:New("steady",               "Ability_Hunter_SteadyShot")
	...
	function lazyHunter.masks.isSwinged()
		if not Quiver or not Quiver.GetSecondsRemainingReload then
			lazyScript.p("Quiver not detected - isSwinged disabled.")
			return false
		end
		
		local reloadTime = select(2, Quiver.GetSecondsRemainingReload())
		if reloadTime < 0.75 then
			return false
		end
		return true
	end
	...
	function lazyScript.bitParsers.ifSwinged(bit, actions, masks)
		...
		table.insert(masks, lazyScript.negWrapper(lazyHunter.masks.isSwinged, negate))
		return true
	end
	...
	function lazyHunter.CustomHelp()
		return [[
			...
			<P>-if[Not]Swinged |cffffff00(need Quiver)|r</P>
		]]
	end
