...
function lazyScript.masks.TargetNPC()
	local target = UnitIsPlayer("target")
...
function lazyScript.masks.Dodged(unitId, op, val, spell)
    return function(sayNothing)
        local ts = lazyScript.lastDodgeTime[unitId] or 0
        local timeOk = lazyScript.masks.EventTime(ts, op, val)
        if not timeOk then return false end
        if spell == "" then return true end
        local lastSpell = lazyScript.lastDodgeSpell[unitId] or ""
        return lastSpell == spell
    end
end

function lazyScript.masks.Parried(unitId, op, val, spell)
    return function(sayNothing)
        local ts = lazyScript.lastParryTime[unitId] or 0
        local timeOk = lazyScript.masks.EventTime(ts, op, val)
        if not timeOk then return false end
        if spell == "" then return true end
        local lastSpell = lazyScript.lastParrySpell[unitId] or ""
        return lastSpell == spell
    end
end

function lazyScript.masks.Blocked(unitId, op, val, spell)
    return function(sayNothing)
        local ts = lazyScript.lastBlockTime[unitId] or 0
        local timeOk = lazyScript.masks.EventTime(ts, op, val)
        if not timeOk then return false end
        if spell == "" then return true end
        local lastSpell = lazyScript.lastBlockSpell[unitId] or ""
        return lastSpell == spell
    end
end

function lazyScript.masks.Resisted(unitId, op, val, spell)
    return function(sayNothing)
        local ts = lazyScript.lastResistTime[unitId] or 0
        local timeOk = lazyScript.masks.EventTime(ts, op, val)
        if not timeOk then return false end
        if spell == "" then return true end
        local lastSpell = lazyScript.lastResistSpell[unitId] or ""
        return lastSpell == spell
    end
end
...
function lazyScript.bitParsers.ifEventTime(bit, actions, masks)
    ...
    local unitId = lazyScript.match2
    if unitId == "" then unitId = "player" else unitId = string.lower(unitId) end

    local event = lazyScript.match3
    local eventFunc = lazyScript.masks[event]

    local op, val, factor = nil, nil, lazyScript.match4
    ...
    if op == nil then op = "<" ; val = 5 end

    local spellCode = lazyScript.match5 or ""
    local spellName = ""
    if spellCode ~= "" then
        spellCode = string.sub(spellCode, 2)
        local action = lazyScript.actions[spellCode] or lazyScript.otherActions[spellCode] or lazyScript.comboActions[spellCode]
        if action then
            local fullName = action.name or spellCode
            spellName = string.gsub(string.lower(fullName), "%s+", "")
        else
            spellName = string.gsub(string.lower(spellCode), "%s+", "")
        end
    end

    table.insert(masks,
        lazyScript.negWrapper(eventFunc(unitId, op, val, spellName), negate))
    return true
end
